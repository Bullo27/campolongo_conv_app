...
def load_clip_results(clip_num: int) -> dict | None:
...
def timeline_to_arrays(timeline: list, duration_ms: float, resolution_ms: float = 32.0):
    """Convert a timeline to a frame-level label array for plotting."""
    n_frames = int(duration_ms / resolution_ms) + 1
    labels = np.zeros(n_frames)  # 0 = silence, 1 = A, -1 = B
    times = np.arange(n_frames) * resolution_ms / 1000  # in seconds
...
def _compute_sb_metrics(result: dict) -> dict:
    """Compute speechbrain-equivalent metrics from its timeline + comparison data."""
...
def visualize_clip(result: dict, label_map: dict | None = None):
    """Generate a multi-panel visualization for one clip."""
...
def _draw_metrics_table(ax, result: dict, trace: list):
    """Draw a metrics comparison table (our pipeline vs speechbrain) on the given axis."""
    ...
    # Build table data
    # Columns: Metric | Our Pipeline | SpeechBrain | Diff
    if has_sb:
    ...
    # Color the difference column (green=close, red=far)
    if n_cols == 4:
        for i, row in enumerate(rows, start=1):
            if row[3] != "—":
...
def _plot_timeline(ax, times, labels, title):
    """Plot a speaker timeline on a matplotlib axis."""
    import matplotlib.patches as mpatches

    # Create colored regions
    colors = {1: "#2196F3", -1: "#FF5722", 0: "#E0E0E0"}  # A=blue, B=orange, silence=gray
...
def main():
    ...
    parser.add_argument("--clips", type=int, nargs="+", default=[1, 2, 3, 4, 5],
                        help="Clip numbers to visualize")
    ...
    for clip_num in args.clips:
        ...
        # Get label map from comparison results
        label_map = None
